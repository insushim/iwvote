...
function hashVoteCode(code: string): string {
  ...
  return CryptoJS.HmacSHA256(normalized, getHmacSecret()).toString(
    CryptoJS.enc.Hex,
  );
}
...
function computeVoteHash(
  ...
  return CryptoJS.SHA256(`${voteData}|${timestamp}|${previousHash}`).toString(
    CryptoJS.enc.Hex,
  );
}
...
function computeBlockHash(
  ...
  return CryptoJS.SHA256(
    `${index}|${timestamp}|${voteHash}|${previousHash}`,
...
export const validateCode = functions.https.onCall(async (data, context) => {
  ...
  if (voterCode.used) {
    throw new functions.https.HttpsError(
      "already-exists",
      "이미 사용된 투표 코드예요. 하나의 코드로 한 번만 투표할 수 있어요.",
    );
  }
...
export const castVote = functions.https.onCall(async (data, context) => {
  ...
  const isValidCandidate = election.candidates.some(
  ...
  const voteHash = CryptoJS.SHA256(`${encryptedVote}|${timestamp}`).toString(
    CryptoJS.enc.Hex,
  );
...
export const getElectionResults = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    const candidateCounts: Record<
      string,
      { total: number; classCounts: Record<string, number> }
    > = {};
    ...
    for (const doc of votesSnap.docs) {
      ...
      if (
        resolvedCandidateId === "__abstention__" ||
        resolvedCandidateId === "abstain" ||
        resolvedCandidateId === ""
      ) {
        abstentions++;
        continue;
    ...
    const candidates = election.candidates.map(
      (c: { id: string; name: string; number: number }) => {
    ...
    const classTurnout: Record<
      string,
      { voted: number; total: number; rate: number }
    > = {};
    // targetClasses가 비어있으면 실제 투표 데이터에서 반 목록 추출
    const classIds =
      election.targetClasses && election.targetClasses.length > 0
        ? election.targetClasses
        : Object.keys(votesByClass).sort();
    for (const classId of classIds) {
      const voted = votesByClass[classId] ?? 0;
      const total = Math.round(totalVoters / (classIds.length || 1));
...
export const onVoteCreated = functions.firestore
  ...
  .onCreate(async (snap) => {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const ts = voteData.timestamp?.toMillis
        ? voteData.timestamp.toMillis()
        : Date.now();
      ...
      transaction.update(electionRef, {
        ...
        hashChainLength:
          (election.hashChainLength ?? election.totalVoted ?? 0) + 1,
      });
    });
  });
...
export const generateVoterCodes = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    // Update totalVoters on election document (count all codes for this election)
    const allCodesSnap = await db
      .collection(COLLECTIONS.VOTER_CODES)
      .where("electionId", "==", electionId)
      .count()
      .get();
    const totalVoters = allCodesSnap.data().count;
    await db
      .collection(COLLECTIONS.ELECTIONS)
      .doc(electionId)
      .update({ totalVoters });
    ...
    await createAuditLog(
      ...
      "codes_generated",
      context.auth.uid,
      `${classId} 반 투표 코드 ${count}개 생성 (총 유권자: ${totalVoters}명)`,
...
export const verifyHashChain = functions.https.onCall(async (data, context) => {
  ...
  if (valid) {
    for (let i = 0; i < blocks.length; i++) {
      ...
      const ts = block.timestamp?.toMillis
...
export const lookupSchoolByJoinCode = functions.https.onCall(
  async (data, context) => {
...
export const claimSuperAdmin = functions.https.onCall(async (data, context) => {
  ...
  if (!userSnap.exists) {
    throw new functions.https.HttpsError(
      "not-found",
      "사용자 프로필을 찾을 수 없습니다. 먼저 회원가입을 완료해주세요.",
    );
  }
