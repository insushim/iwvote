...
export default function AdminRegisterPage() {
  ...
  // After successful signup, redirect to login page (or dashboard for superAdmin)
  useEffect(() => {
    ...
    // For new school admin or pending users, redirect to login after brief delay
  ...
  const handleLookupJoinCode = async () => {
    ...
    try {
      ...
      >(functions, "lookupSchoolByJoinCode");
      ...
      const message =
        err instanceof Error ? err.message : "가입 코드를 찾을 수 없습니다.";
      setError(message);
      setJoinedSchoolId("");
      setJoinedSchoolName("");
    } finally {
  ...
  // After successful signup, show completion screen
  if (signUpComplete) {
    const isPendingUser = isPending;
    return (
        ...
        >
          <div className="rounded-2xl bg-white p-8 shadow-2xl">
            <div className="mb-6 text-center">
              <div
                className={`mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl ${isPendingUser ? "bg-yellow-50" : "bg-green-50"}`}
              >
                {isPendingUser ? (
              ...
              <h1 className="text-2xl font-bold text-gray-900">
                {isPendingUser ? "승인 대기중" : "가입 완료!"}
              </h1>
            </div>

            <div
              className={`rounded-lg px-4 py-4 text-center text-sm ${isPendingUser ? "bg-yellow-50 text-yellow-800" : "bg-green-50 text-green-800"}`}
            >
              {isPendingUser
